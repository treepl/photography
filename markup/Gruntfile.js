module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    sass: {
      ...
      dev: {
        ...
        files: [{
            expand: true,
            cwd: '<%= config.sassDir %>',
            src: ['<%= config.srcDir %>**/*.scss', '!<%= config.sassMainFileName %>.scss'],
            dest: '<%= config.cssDir %>',
            ext: '.css'
          },
          {
    ...
    browserSync: {
      dev: {
        bsFiles: {
          src: ['dist/*.html', '<%= config.cssDir %>*.css', '*.css']
        },
        options: {
          server: {
            baseDir: "dist/",
  ...
  //finally
  //css beautiful
  grunt.registerTask('cssbeauty', ['sass:min', //'cmq:dist',
    'postcss:dist', 'csscomb:dist'
  ]);
  //img minify
